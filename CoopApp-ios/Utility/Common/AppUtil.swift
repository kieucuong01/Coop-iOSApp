...
//
//  Created by Cuong Kieu on 7/5/18.
...
final class AppUtil {
    ...
    // Fonts
    static let mainFont: UIFont = UIFont.systemFont(ofSize: 15.0 * DISPLAY_SCALE)
    ...
    static func getStringDate(from milisecondsDouble: Double?) -> String? {
        if let miliseconds: Double = milisecondsDouble {
            ...
            if abs(secondsFromNow) <= secondsOneDay {
                ...
                // Still in this year
            else if AppUtil.isDateOfThisYear(of: date) == true {
    ...
    static func showTopErrorView(message : String){
        if let currentVC = Util.topViewController() as? BaseViewController {
            if currentVC is SignInVC {
                currentVC.showTintTopError(message: message)
            }
            else {
                currentVC.showTopError(message: message)
            }
        }
    }
