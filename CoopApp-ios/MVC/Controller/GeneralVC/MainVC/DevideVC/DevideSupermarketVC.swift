...
class DevideSupermarketVC: BaseViewController {
    ...
    private func callAPIList(isReload: Bool, searchKey: String?, branchTypeId: Int?) {
        ...
        if let transaction = self.data[TransactionModel.keyModel] as? TransactionModel {
            params["product_id"] = transaction.product?.id
        }
        ...
        APIBase.sharedInstance()?.callAPIGetBranch(isShowHUD: true, params: params, completionHandler: { (result, error) in
            ...
            if error == nil && result != nil{
                ...
                if result?["success"] as! Bool == true {
                    ...
                    for dic in dics {
                        self.list.append( BranchModel(dict: dic["branch"] as! Dictionary<String, Any>))
                    }
...
extension DevideSupermarketVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell: SupermarketTVC = tableView.dequeueReusableCell(withIdentifier: "supermarketTVC") as? SupermarketTVC {
            ...
            if self.list.indices.contains(indexPath.row){
                let model : BranchModel = self.list[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let branch : BranchModel = self.list[indexPath.row]
        if let transaction =  self.data[TransactionModel.keyModel] as? TransactionModel {
            self.pushToViewController(storyboardName: "Devide", identifier: "devideReceiverVC", animated: true, withParameter: ["branch" : branch, TransactionModel.keyModel : transaction])
        }
    }    
}

extension DevideSupermarketVC : UISearchBarDelegate {
