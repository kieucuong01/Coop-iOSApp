...
protocol OccupancyRateChartVCDelegate {
    func chartTranslated(chartView: BarChartView)
}
class OccupancyRateChartVC: BaseViewController {
    
    @IBOutlet var chartView: BarChartView!
    var parentView : MonthyReportTVC?
    var chartData : [ChartDataModel] = [ChartDataModel]()
    var delegate : OccupancyRateChartVCDelegate? = nil
    ...
    private func addObserverNotification(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.chartDrag(notification:)), name: NSNotification.Name("ResidentChartDrag"), object: nil)
    }
    ...
    private func initChartView(){
        ...
        let marker = OccupancyRateChartMarker(color: PRIMARY_COLOR, font: UIFont(name: PRIMARY_FONT_NAME_MEDIUM, size: 17)!, textColor: .white)
...
extension OccupancyRateChartVC : ChartViewDelegate {
    func chartTranslated(_ chartView: ChartViewBase, dX: CGFloat, dY: CGFloat) {
        self.delegate?.chartTranslated(chartView: self.chartView)
        NotificationCenter.default.post(name: NSNotification.Name("OccupancyRateChartDrag"), object: self.chartView.lowestVisibleX)
    }
